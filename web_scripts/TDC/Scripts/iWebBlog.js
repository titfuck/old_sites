...
{this.title=getChildElementTextByTagName(itemNode,"title");this.p_linkText=getChildElementTextByTagName(itemNode,"link");this.description=getChildElementTextByTagName(itemNode,"description");this.absoluteURL=new IWURL(this.p_linkText);this.date=new Date();var dateText=getChildElementTextByTagName(itemNode,"pubDate");if(dateText&&dateText.length>0)
this.date=new Date(dateText);var image=BlogFeed.getiWebElement(itemNode,"image");if(image)
{this.imageUrlString=image.getAttribute("href");this.imageURL=new IWURL(this.imageUrlString);}
this.commentingEnabled=false;this.commentCount=0;var iwebCommentElement=BlogFeed.getiWebElement(itemNode,"comment");if(iwebCommentElement)
{this.commentCount=iwebCommentElement.getAttribute("count");this.commentingEnabled=(iwebCommentElement.getAttribute("enabled")==1);this.commentURL=new IWURL(iwebCommentElement.getAttribute("link"));}}});Object.extend(BlogFeed.prototype,{p_parseFeed:function(rssDoc)
